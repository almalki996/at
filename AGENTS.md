...
16. **ZERO-TRUST SECURITY (FRONTEND)**: NEVER place Directus Admin Tokens inside `.env` variables prefixed with `NEXT_PUBLIC_`. All frontend schema/data fetching hooks (e.g., `useDirectusSchema`) MUST rely strictly on `localStorage.getItem("directus_token")` provided dynamically by the user's authentic session.
17. **RELATIONAL DATA HYDRATION**: To prevent GenericList tables from awkwardly displaying raw `id` UUIDs or numbers for M2O relations, ALWAYS ensure `fields=*.*,*` is injected into the GET requests either via the `@refinedev/simple-rest` `meta: { fields: ['*', '*.*'] }` syntax or intercepted natively via `data-provider.ts` Axios config.
18. **FORM AESTHETICS (DATEPICKER)**: DO NOT use the native browser `<input type="date">` as it violates the premium aesthetic style. ALWAYS import and utilize the customized `DatePickerField` wrapper component built around `react-datepicker` for cohesive Tailwind UI consistency.
