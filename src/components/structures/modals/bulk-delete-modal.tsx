...
export interface StructureBulkDeleteModalProps {
    isOpen: boolean;
    onClose: () => void;
...
export const StructureBulkDeleteModal = ({ isOpen, onClose, selectedCount, onConfirm }: StructureBulkDeleteModalProps) => {
    if (!isOpen) return null;

    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm transition-opacity" onClick={onClose}></div>
            <div className="relative bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-md p-8 animate-in fade-in zoom-in-95 duration-200 border border-white/20 dark:border-slate-800 transition-colors rtl">
                ...
                <div className="text-gray-600 dark:text-slate-300 mb-8 font-medium leading-relaxed">
                    هل أنت متأكد من رغبتك في حذف <span className="font-bold text-rose-600 dark:text-rose-400">({selectedCount}) أقسام</span> محددة؟ 
                    <span className="text-sm font-bold text-red-500 mt-3 bg-red-50 dark:bg-red-900/20 p-3 rounded-lg block border border-red-100 dark:border-red-900/50">
                        ⚠️ سيتم حذفهم مع كافة الأقسام الفرعية التابعة لهم نهائياً ولن تتمكن من الاسترجاع.
                    </span>
                </div>
